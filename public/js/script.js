...
(function () {
  ...
  var I18N = {
    en: {
      ...
      'story.eyebrow':'our story','story.accent':'where two became one',
      'story.lead':"From a first hello to a forever yes — every moment led us here, and we'd choose each other all over again.",
      ...
      'rsvp.eyebrow':'kindly reply by 20 June','rsvp.title':'Will you join us?',
    ...
    ru: {
      ...
      'story.eyebrow':'наша история','story.accent':'когда двое стали одним',
      'story.lead':'От первого «привет» до вечного «да» — каждый миг вёл нас сюда, и мы снова выбрали бы друг друга.',
      ...
      'rsvp.eyebrow':'просим ответить до 20 июня','rsvp.title':'Присоединитесь к нам?',
    ...
    hy: {
      ...
      'story.eyebrow':'մեր պատմությունը','story.accent':'երբ երկուսը դարձան մեկ',
      'story.lead':'Առաջին «բարև»-ից մինչև հավերժ «այո»՝ ամեն պահ մեզ բերեց այստեղ, և մենք նորից կընտրեինք միմյանց։',
      ...
      'rsvp.eyebrow':'խնդրում ենք պատասխանել մինչև հունիսի 20-ը','rsvp.title':'Կմիանա՞ք մեզ',
  ...
  var RSVP_ENDPOINT = 'https://script.google.com/macros/s/AKfycbyEJK7r-dxmYq1ZZUHxDFSIxlLdXKNRmVwEq1gbR1s5qE_H6tbi7lts4ZD9ukmbdDPjbg/exec';
  ...
  if (car && track) {
    var GAP = 20, pos = 0, half = 0, paused = false, drag = false, sx = 0, sp = 0, moved = 0, slideTo = null;
    function measure() { half = (track.scrollWidth + GAP) / 2; }
    function render() { track.style.transform = 'translateX(' + (-pos) + 'px)'; }
    function wrap() {
      if (half > 0) {
        if (pos >= half) { pos -= half; if (slideTo !== null) slideTo -= half; }
        else if (pos < 0) { pos += half; if (slideTo !== null) slideTo += half; }
      }
    }
    ...
    function nudge(dir) {
      paused = true;
      slideTo = (slideTo === null ? pos : slideTo) + dir * stepW();
    ...
    var loop = function () {
      if (visible) {
        if (slideTo !== null) {                 // ease toward button target
          var d = slideTo - pos;
          if (Math.abs(d) < 0.5) { pos = slideTo; slideTo = null; }
          else pos += d * 0.16;
          wrap(); render();
